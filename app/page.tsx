...
export default function DashboardPage(){
 ...
 const total=tasks.length,inProgress=tasks.filter(t=>t.status==='진행중').length,completed=tasks.filter(t=>t.status==='완료').length,overdue=tasks.filter(t=>isOverdue(t)).length;
 ...
 return <MotionConfig reducedMotion="user"><StudioShell member={member} tasks={tasks} active={view} onNavigate={navigate} onSwitchMember={switchMember}>
  ...
  {loading?<div className="studio-skeleton" aria-label="업무 불러오는 중" role="status"><div/><div/><div/><div/><p>팀의 업무 현황을 불러오고 있습니다.</p></div>:<>
   <section hidden={view!=='overview'} className="studio-overview">
    ...
    <SummaryCards total={total} inProgress={inProgress} completed={completed} overdue={overdue} onSelect={status=>focusTasks({member:'all',...(status==='지연'?{overdue:true}:status==='전체'?{}:{status})})}/>
