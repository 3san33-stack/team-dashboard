...
export default function DashboardPage(){
 ...
 async function handleQuickUpdate(id:string,patch:Partial<TaskInput>){const t=tasks.find(x=>x.id===id);if(!t)return;const {member,project,category,detail,priority,start_date,due_date,progress,status,comment}=t;try{await updateTask(id,{member,project,category,detail,priority,start_date,due_date,progress,status,comment,...patch});setActionError(null);await refresh()}catch{setActionError('업무를 수정하지 못했습니다. 다시 시도해 주세요.')}}
 ...
 const onHold=tasks.filter(t=>t.status==='보류'||t.status==='검토중').length,total=tasks.length-onHold,inProgress=tasks.filter(t=>t.status==='진행중').length,completed=tasks.filter(t=>t.status==='완료').length,planned=tasks.filter(t=>t.status==='예정').length,overdue=tasks.filter(t=>isOverdue(t)).length;
 ...
 return <MotionConfig reducedMotion="user"><StudioShell member={member} tasks={tasks} active={view} onNavigate={navigate} onSwitchMember={switchMember}>
  ...
  {loading?<div className="studio-skeleton" aria-label="업무 불러오는 중" role="status"><div/><div/><div/><div/><p>팀의 업무 현황을 불러오고 있습니다.</p></div>:<>
   <section hidden={view!=='overview'} className="studio-overview">
    ...
    <SummaryCards total={total} inProgress={inProgress} completed={completed} planned={planned} onHold={onHold} overdue={overdue} onSelect={status=>focusTasks({member:'all',...(status==='지연'?{overdue:true}:status==='전체'?{status:'보류·검토중 제외'}:{status})})}/>
    <div className="studio-insight-grid"><section className="studio-panel"><div className="studio-panel-heading"><div><span className="studio-eyebrow">TEAM MOMENTUM</span><h2>함께 만드는 진척</h2></div><AnalyticsDialog tasks={tasks} trigger={<button className="studio-link">팀 분석 <ArrowUpRight size={16}/></button>}/></div><div className="studio-team-list">{MEMBERS.map((m,i)=>{const mine=tasks.filter(t=>t.member===m),done=mine.filter(t=>t.status==='완료').length,progress=averageProgress(tasks,m);return <button key={m} className="studio-team-row" onClick={()=>focusTasks({member:m})}><MemberAvatar member={m} index={i}/><span className="studio-team-name">{m}<small>{mine.length}건 · 완료 {done}건</small></span><span className="studio-team-track"><span style={{width:progress+'%'}}/></span><strong>{progress}%</strong><ChevronRight size={15}/></button>})}</div><div className="studio-panel-foot"><CheckCheck size={16}/><span>팀 전체 완료율 <strong>{completion}%</strong></span><span>{completed} / {total}건 완료</span></div></section><section className="studio-panel studio-deadlines"><div className="studio-panel-heading"><div><span className="studio-eyebrow">NEEDS YOUR ATTENTION</span><h2>먼저 확인할 업무</h2></div><span className="studio-soft-icon"><Clock3 size={19}/></span></div>{deadlines.length?<div className="studio-deadline-list">{deadlines.map(task=><button key={task.id} onClick={()=>setEditingTask(task)}><span className={isOverdue(task)?'studio-due late':'studio-due'}>{isOverdue(task)?'지연':'마감'}</span><span><strong>{task.project}</strong><small>{task.member} · {task.due_date}</small></span><ArrowUpRight size={16}/></button>)}</div>:<div className="studio-calm"><CheckCheck size={32}/><p>예정된 마감이 없습니다.</p><span>다음 작업을 차분히 준비해보세요.</span></div>}<button className="studio-link studio-full-link" onClick={()=>focusTasks({member:'all',overdue:true})}>지연 업무 {overdue}건 확인 <ArrowUpRight size={15}/></button></section></div>
