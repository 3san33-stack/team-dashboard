...
// 전체 건수와 완료율은 보류를 제외한다. 검토중은 전체에 포함한다.
...
export function SummaryCards({total,inProgress,completed,planned,onHold,overdue,onSelect}:Props){
 const statusItems=[{label:'전체 업무',key:'전체',value:total,icon:Layers3,note:'보류를 제외한 업무 · 검토중 포함'},{label:'진행중',key:'진행중',value:inProgress,icon:LoaderCircle,note:'현재 진행 중인 작업'},{label:'완료',key:'완료',value:completed,icon:CheckCheck,note:total?`전체 업무의 ${Math.round(completed/total*100)}% 완료`:'등록된 업무가 없습니다'},{label:'예정',key:'예정',value:planned,icon:CalendarClock,note:'아직 시작하지 않은 업무'}];
 return <div className="studio-summary">
  ...
  <div className="studio-summary-separate"><button className="studio-stat studio-stat-overdue" onClick={()=>onSelect?.('보류')} disabled={!onSelect}><div className="studio-stat-label">보류<PauseCircle size={19}/></div><div className="studio-stat-number"><CountUpNumber value={onHold}/><span>건</span></div><div className="studio-stat-note"><span>전체 건수·완료율에서 제외</span><ArrowUpRight size={15}/></div></button>
